...
|---------|------|-------------|
| `utilities_rhel_facts_tuned_active` | bool | Whether `tuned` is active. |
| `utilities_rhel_facts_fw_active` | bool | Whether `firewalld` is active. |
| `utilities_rhel_facts_used_mb`, `utilities_rhel_facts_total_mb` | int | Memory in MB (used/total). |
| `utilities_rhel_facts_used_g`, `rhel_facts_total_g` | float | Memory in GiB (used/total). |
| `utilities_rhel_facts_rel_line` | string | Raw `/etc/redhat-release` line. |
| `utilities_rhel_facts_rel_sane` | string | Normalized release string (e.g., `RedHat 9.4`). |
| `utilities_rhel_facts_time_iso` | string | UTC time in ISO‑8601. |
| `utilities_rhel_facts_hostname` | string | FQDN (fallback to short hostname). |
| `utilities_rhel_facts_ip_cidr` | string | Primary IPv4 in CIDR (e.g., `10.0.0.10/24`). |
| `utilities_rhel_facts_ip_only` | string | IPv4 address only. |
| `utilities_rhel_facts_prefix` | string | CIDR prefix length (e.g., `24` or empty). |
| `utilities_rhel_facts_gw_line` | string | Default route line from `ip route`. |
| `utilities_rhel_facts_dns_list` | list[str] | DNS nameservers from `/etc/resolv.conf`. |
